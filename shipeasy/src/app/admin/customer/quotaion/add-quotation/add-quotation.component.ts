...
export class AddQuotationComponent implements OnInit {
  ...
  onLocationSearch(e) {
    ...
    if (shipmentType.toLowerCase() == 'air') {
      ...
      this.locationData = [...this.cityList]?.map(x => ({
  ...
  constructor(private _api: CommonService, private _modal: NgbModal, public _cognito: CognitoService,public commonFunction : CommonFunctions,
    private router: Router, private fb: FormBuilder,   public notification: NzNotificationService,
  ) {
...
this.getLocationDropDowns()
  ...
  setselect(e){
    ...
    if (selectedType?.typeName === 'Air') {
      this.errorMessage = 'Please Select AirPort';
      ...
      this.errorMessage = 'Please Select Port';
      this.placeholderText = 'Enter Port';
      this.errorMessagedest = this.errorMessage;
    }
  }
  CancelLoad(){
    this.isDropdownOpen =false;
  ...
  handleChangeFilter = ($event: any, catagory: string, name: any) => {
    ...
    let dataObj: any = {}
    if(catagory === 'customsclearance'){
      dataObj[name.typeName] = $event?.target?.checked
    }
    else{
    ...
    if (this.selectedFilter?.customsclearance) { 
      ...
      if (!showAll)
        data = data?.filter(item => {
          console.log(data)
          if (item.isCustomDestination && this.selectedFilter?.customsclearance?.["Destination"]) {
            return true
          }
          if (item.isCustomOrigin && this.selectedFilter?.customsclearance?.["Origin"]) {
            return true
          }
          return false
        })
  ...
  sendQuotation(selectedRow?) {
    ...
    this.branchs?.controls.forEach((element) => {
      let details = {
        units: Number(element.value.quantity) || 0,
    ...
    this._api.addToST('enquiry', newAgentAdvice)?.subscribe((res: any) => {
      if (res) {
        ...
        this.router.navigate(['/customer/quotation/Export/list']);
        this.commonFunction.pageNo = 0;
      }
    });
  }
