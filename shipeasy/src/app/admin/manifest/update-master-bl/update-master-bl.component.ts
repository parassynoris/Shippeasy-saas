...
@Component({
  selector: 'app-update-master-bl',
  templateUrl: './update-master-bl.component.html',
  styleUrls: ['./update-master-bl.component.scss']
})
export class UpdateMasterBlComponent implements OnInit {
  houseBlList: any = []
  vesseldata: any;
  voyageData: any;
  tankData: any = [];
  ...
  getShippingLineDropDowns() { 
      ...
      if(payload?.query)payload.query = {   "status": true, "$and": [
        {
          "feeder": {
            "$ne": true
          }
  ...
  getBLById() { 
      ...
      if(payload?.query)payload.query = {   "vessel": this.route.snapshot.params['vesId'], "voyageId": this.route.snapshot.params['voyId'],"isExport": true }
      if(payload?.query)payload.query['blType'] = "MBL"
