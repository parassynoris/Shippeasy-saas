...
describe('ManifestListComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockActivatedRoute = {
      snapshot: {
        params: {
          vesId: 'yourVesId', // Provide your test vesId value
          voyId: 'yourVoyId' // Provide your test voyId value
        }
      }
    };
    await TestBed.configureTestingModule({
      declarations: [ ManifestListComponent ,MastersSortPipe],
  ...
  it('should fetch BL list based on vessel and voyage ID', () => {
    const filterListReturnValue = { query: {} };
    const getSTListReturnValue = { documents: [{}, {}] };

    mockCommonService.filterList.and.returnValue(filterListReturnValue);
    mockCommonService.getSTList.and.returnValue(of(getSTListReturnValue));

    component.getBLById();

    expect(mockCommonService.filterList).toHaveBeenCalled();
    expect(mockCommonService.getSTList).toHaveBeenCalledWith('bl', filterListReturnValue);
    expect(component.houseBlList).toEqual(getSTListReturnValue.documents);
  });
