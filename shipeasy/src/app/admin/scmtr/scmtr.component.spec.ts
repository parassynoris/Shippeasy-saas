...
const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => ScmtrComponent),
  multi: true
};

describe('ScmtrComponent', () => {
  let component: ScmtrComponent;
  let fixture: ComponentFixture<ScmtrComponent>;
  ...
  let mockCommonService: jasmine.SpyObj<CommonService>;
    ...
    TestBed.configureTestingModule({
      declarations: [ScmtrComponent, MockTranslatePipe,],
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(ScmtrComponent);
  ...
  it('should initialize tabs with translated values', () => {
    spyOn(component.translate, 'get').and.returnValue(of({ scmtr: { tab: { vessel: 'Translated Vessel' } } }));
    
    component.ngOnInit();
  
    expect(component.tabs[0].name).toEqual('Translated Vessel');
  });
  it('should navigate to the correct route and update holdControl', () => {
    const data = { key: 'some_key' };
    const routerSpy = spyOn(component.router, 'navigate').and.callThrough();
    
    component.onTab(data);
    
    expect(routerSpy).toHaveBeenCalledWith(['/scmtr/some_key']);
    expect(component.holdControl).toEqual('some_key');
  });
