...
export class BatchComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Load saved milestone filter from localStorage
    if (localStorage.getItem('milestone') != '') {
      this.selectedMilestone = localStorage.getItem('milestone') || '';
    }
  
    // Load saved consignee filter from localStorage
    const savedConsignee = localStorage.getItem('selectedConsignee');
    if (savedConsignee) {
      this.selectedConsignee = JSON.parse(savedConsignee);
    }
  
    // Load other saved filter states
    ...
    // Set default batch type
    this.holdBatchType = "export";
    this.appCommon.updateMessage(this.holdBatchType);
  
    // Check query params for special show mode
    this.route.queryParams.subscribe(params => {
      this.isshow = params['true'];
      this.applyUrlParamsToFilters(params);
    });
  
    // Load batch list immediately - NO setTimeout
    // Check if there's a filter status from query params
  ...
  applyUrlParamsToFilters(params: any): void {
    this.filtersModel = [];
    ...
    this.displayedColumns.forEach((column, index) => {
      ...
      switch (column) {
        case 'enquiryDetails.basicDetails.ShipmentTypeName':
          if (params['shipmentType']) {
            filterValue = params['shipmentType'];
          }
          break;
  
        case 'enquiryDetails.basicDetails.loadType':
          if (params['loadType']) {
            filterValue = params['loadType'];
          }
          break;
      }
  
      this.filtersModel[index] = filterValue;
    });
  
    const hasFilters = this.filtersModel.some(filter => filter !== null && filter !== undefined);
  ...
  onScroll(event: any): void {
    ...
    if (this.batchList.length < this.toalLength) {
  ...
  onMilestoneChange(value: string): void {
    ...
    if (value) {
      // this.filterBatch = ['statusOfBatch'] 
      ...
      if (urlParams['shipmentType']) {
        parameter.query['enquiryDetails.basicDetails.ShipmentTypeName'] = urlParams['shipmentType'];
      }
  
      if (urlParams['loadType']) {
        parameter.query['enquiryDetails.basicDetails.loadType'] = urlParams['loadType'];
      }
    ...
    if (this.selectedConsignee && this.selectedConsignee.length > 0) {
      ...
      parameter.query['$and'].push({
        "$or": [
          ...
          // { "enquiryDetails.routeDetails.shippingLineId": { "$in": this.selectedConsignee } }
        ]
      });
    }
    ...
    this._api.getSTList(Constant.BATCH, parameter)
      ?.subscribe((data: any) => {
        ...
        if (this.selectedStatus === 'MBL Telex Pending') {
          filteredDocs = data.documents.filter((batch: any) =>
        ...
        const uniqueDocuments = filteredDocs
      .map(doc => ({ doc, isUnique: !existingBatchIds.has(doc.batchId) }))
      ...
      .map(item => item.doc);
        ...
        this.toalLength = filteredDocs.totalCount;
        this.count = filteredDocs.length;
