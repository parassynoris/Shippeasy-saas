...
@Component({
  selector: 'app-lorray-table',
  templateUrl: './lorray-table.component.html',
  styleUrls: ['./lorray-table.component.scss']
})

export class LorrayTableComponent implements OnInit {
    ...
    this.commonService.getSTList('lorryreceipt', payload)?.subscribe({
      next: (res: any) => {
        ...
        if (res?.documents && res.documents.length > 0) {
           const filteredDocs = res.documents.filter(
          (item: any) => item.batchId === batchId
        );

          this.lorryReceiptList = filteredDocs.map((item: any) => ({
            ...item,
