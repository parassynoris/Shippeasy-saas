...
@Component({
  ...
  templateUrl: './add-receipt-payment.component.html',
  styleUrls: ['./add-receipt-payment.component.scss']
})
export class AddReceiptPaymentComponent implements OnInit {
  ...
  getBankList(e?) {
    ...
    this.commonService.getSTList('bank', payload)
      ?.subscribe((data) => {
        this.bankList = data?.documents?.filter((x) => (x?.branchId == this.f.billToBranch.value || x.isDefaultBank));

        this.defaultBanks = data?.documents
        ...
        let data123 = this.bankList?.filter((x) => this.newReceiptForm.value.banks.includes(x?.bankName))
  ...
  formBuild(data?) {
    this.newReceiptForm = this.formBuilder.group({
      ...
      batchId: [data ? data?.batchId : this.isBatchInvoice ? this.batchId || '' : '', data?.withoutJob ? '' : (this.isBatchInvoice || this.type == 'sellerInvoice') ? Validators.required : ''],

      batchArray: [data ? data?.batchArray?.map((x) => { return x.batchId }) || [] : [], data?.withoutJob ? '' : !this.isBatchInvoice && this.type == 'buyerInvoice' ? Validators.required : ''],
      ...
      banks: [data ? [data?.bankName] : this.defaultBanks, this.type == 'sellerInvoice' ? Validators.required : ''],
      ...
      invoiceRefNo: [data ? data?.invoiceNo : '', this.type == 'buyerInvoice' ? Validators.required : ''],
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.getBankList();
    await this.getCustomerParty();
    this.getStateList();
    this.getpaymentMode();
    await this.getBatchList(); // Wait for batch list
    ...
    if (this.idToUpdate) {
      this.getRecieptList();
    } else if (this.isBatchInvoice && this.batchId) {
      // Auto-trigger getBatchDetails for batch invoice
      setTimeout(() => {
  ...
  customerList: any = []
  async getCustomerParty(): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.commonService.getSTList('partymaster', payload)?.subscribe((res: any) => {
        this.customerList = res?.documents?.filter((x) => x.isSupplier);
        resolve();
      });
    });
  }
  ...
  getListOfbillFrom(e?) {
    ...
    console.log(this.billFromBranchList, 'billFromBranchList  2');
  ...
  getBatchListFromParty(e) {
    ...
    shouldArray.push(
      ...
      { "enquiryDetails.basicDetails.multiShipper.name": { "$regex": query, "$options": "i" } },
      { "enquiryDetails.basicDetails.multiConsignee.name": { "$regex": query, "$options": "i" } },
    )
  ...
  async getBatchList(): Promise<void> {
    ...
    return new Promise((resolve) => {
      this.commonService.getSTList('batch', payload)?.subscribe((data: any) => {
        ...
        if (this.type === "sellerInvoice" && this.isType !== 'edit') {
          this.newReceiptForm.patchValue({
            customer: this.batchList[0]?.enquiryDetails?.basicDetails?.billingPartyId,
            billFromBranch: this.batchList[0]?.enquiryDetails?.basicDetails?.billingBranch,
          })
          this.billFromBranchList = this.customerList?.find((x) => x.partymasterId === this.newReceiptForm.controls['customer'].value)?.branch || [];
        }

        if (this.isBatchInvoice && this.batchId) {
          this.newReceiptForm.get('batchId').setValue(this.batchId);
        }

        if (this.newReceiptForm.value.batchId) {
  ...
  getBatchDetails(e?) {
    if (this.multiBatch) {
      // Clear MBL/HBL lists when no batches are selected
      if (!this.newReceiptForm.value.batchArray || this.newReceiptForm.value.batchArray.length === 0) {
        this.mblList = [];
        this.hblList = [];
        this.blData = [];
        this.newReceiptForm.patchValue({
      ...
      // Multi-batch BL data fetching logic
      let payload = this.commonService.filterList();
      payload.query = {
        "batchId": {
      ...
      this.commonService.getSTList('bl', payload).subscribe(
        (data: any) => {
          this.blData = data.documents || [];
          this.mblList = this.blData?.filter((x) => (x?.blType == 'MBL' || x?.blType == 'AWB')) || [];
          this.hblList = this.blData?.filter((x) => (x?.blType == 'HBL' || x?.blType == 'HAWB')) || [];

          // Clear previously selected MBL/HBL that no longer exist
          const currentMblValues = this.newReceiptForm.value.mbl || [];
          const validMblValues = currentMblValues.filter(mbl =>
            this.mblList.some(item => item.blNumber === mbl)
          );
          if (validMblValues.length !== currentMblValues.length) {
            this.newReceiptForm.patchValue({ mbl: validMblValues });
          }

          const currentHblValues = this.newReceiptForm.value.hbl || [];
          const validHblValues = currentHblValues.filter(hbl =>
            this.hblList.some(item => item.blNumber === hbl)
          );
          if (validHblValues.length !== currentHblValues.length) {
            this.newReceiptForm.patchValue({ hbl: validHblValues });
          }
        },
        (error) => {
          this.mblList = [];
      ...
      // Single batch logic (existing code)
      ...
      if (this.batchDetails?.blData && this.batchDetails.blData.length > 0) {
        this.blData = this.batchDetails.blData;
        this.mblList = this.batchDetails.blData?.filter((x) => (x?.blType == 'MBL' || x?.blType == 'AWB')) || [];
        this.hblList = this.batchDetails.blData?.filter((x) => (x?.blType == 'HBL' || x?.blType == 'HAWB')) || [];

        if (this.mblList.length === 1 && !this.idToUpdate) {
          this.newReceiptForm.patchValue({
            mbl: [this.mblList[0].blNumber]
          });
        }
        if (this.hblList.length === 1 && !this.idToUpdate) {
          this.newReceiptForm.patchValue({
            hbl: [this.hblList[0].blNumber]
          });
        }
      } else {
        this.getblData();
      ...
      if (!this.multiBatch) {
        this.newReceiptForm.patchValue({
          billToBranch: this.paymentData?.branchId || this.batchDetails?.quotationDetails?.branchId,
          shipper: this.enquiryDetails?.basicDetails?.shipperName,
          consignee: this.enquiryDetails?.basicDetails?.consigneeName,
          shippingLine: this.isTransport ? this.batchDetails?.quotationDetails?.carrierName : this.enquiryDetails?.routeDetails?.shippingLineName || this.batchDetails?.quotationDetails?.carrierName,
      ...
      if (!this.idToUpdate && this.uploadFile) {
        this.costItem();
        this.addChargeOnTable(true);
      }
      else if (!this.idToUpdate) {
        this.getCharges();
      } else {
        this.addChargeOnTable(true);
      }
    ...
    if (this.multiBatch) {
      ...
      this.batchList?.filter((x) => {
        if (this.newReceiptForm.value.batchArray.includes(x?.batchId)) {
          if (x?.quickJob) {
            if (x?.batchId)
              batchData.push(x?.batchId)
          } else {
            if (x?.quotationId)
              quoteData.push(x?.quotationId)
          }
        }
      })
  ...
  getblData() {
    ...
    // First check if blData already exists in batchDetails
    if (this.batchDetails?.blData && this.batchDetails.blData.length > 0) {
      this.blData = this.batchDetails.blData;
      this.mblList = this.batchDetails.blData?.filter((x) => (x?.blType == 'MBL' || x?.blType == 'AWB')) || [];
      this.hblList = this.batchDetails.blData?.filter((x) => (x?.blType == 'HBL' || x?.blType == 'HAWB')) || [];
  ...
  saveCharge(res) {
    ...
    this.getChargeControls().forEach(element => {
      ...
      let batchDetails = this.batchListCopy?.find((x) => x?.batchId == element.value.batchId) || {}
      ...
      if (element.value.enquiryitemId == '' || element.value.enquiryitemId == null) {
        let charge = {
          ...
          vesselName: batchDetails?.routeDetails?.finalVesselName?.toString() || '',
          voyageName: batchDetails?.routeDetails?.finalVoyageName?.toString() || '',
          moveNumber: batchDetails?.moveNo?.toString() || '',
          enquiryNumber: (this.isExport || this.isTransport) ? batchDetails?.enquiryNo : batchDetails?.uniqueRefNo,
          ...
          "buyEstimates": {
            ...
            isReceiptCreated: this.type === 'buyerInvoice' ? true : element.value.isReceiptCreatedBuy || false,
          },
          "selEstimates": {
            ...
            isReceiptCreated: this.type === 'sellerInvoice' ? true : element.value.isReceiptCreatedSell || false,
          },
          "tax": [
            {
  ...
  getCustomerDetails() {
    ...
    this.newReceiptForm?.get("stateOfSupply").setValue(customerData?.placeofSupply)
  ...
  onSave(flag) {
    ...
    const newInvoice = {
      ...
      "invoiceDueDate": this.type == 'buyerInvoice' ? this.newReceiptForm.value.invoiceDueDate : currentTime(this.getFutureDate(6)),
      "invoiceTypeStatus": this.type,
      "type": this.type,
      ...
      "invoiceNo": this.type == 'buyerInvoice' ? this.newReceiptForm.value.invoiceRefNo : "",
      ...
      batchArray: this.isBatchInvoice && this.type == 'buyerInvoice' ?
        [{
          "batchNo": this.batchList.filter((x) => x?.batchId === this.newReceiptForm.value.batchId)[0]?.batchNo,
          "batchId": this.newReceiptForm.value.batchId
        }]
        : this.batchList?.filter((x) => this.newReceiptForm.value.batchArray.includes(x?.batchId)).map((m) => {
          return {
            batchId: m?.batchId,
            batchNo: m?.batchNo
          }
    ...
    if (this.idToUpdate && this.isType === 'edit') {
      this.commonService.UpdateToST(`invoice/${this.idToUpdate}`, {
        ...newInvoice, invoiceId: this.idToUpdate, invoiceNo:
          this.type == 'buyerInvoice' ? this.newReceiptForm.value.invoiceRefNo : this.paymentData?.invoiceNo
      }).subscribe(
        (data: any) => {
          if (data) {
  ...
  readFile(openError) {
    ...
    if (this.uploadFile) {
      ...
      this.commonService.uploadDocumentsBL('scan-p-invoice', formData).subscribe(res => {
        this.paymentData = res?.invoiceObject;
        this.type = this.paymentData.type;
        this.formBuild(this.paymentData);
        this.costItemList = this.paymentData?.costItems;
        this.baseCostItems = this.paymentData?.costItems;
