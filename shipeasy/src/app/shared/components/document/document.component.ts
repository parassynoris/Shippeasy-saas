...
ngOnChanges(changes: SimpleChanges): void {
  ...
  //     this.commonService.getSTList('instruction', payload)?.subscribe((res: any) => {
  //       if (res?.documents?.length > 0) {
  //         // Extract only the required fields, including siAttachmentName
  //         this.instructionData = res.documents.map((doc: any) => ({
  //           dgAttachmentName: doc?.dg?.dgAttachmentName,
  //           firstPrintAttachmentName: doc?.mblDraft?.firstPrintAttachmentName,
  //           finalAttachmentName: doc?.mblOriginal?.finalAttachmentName,
  //           filedAttachmentName: doc?.si?.filedAttachmentName,
  //           siAttachmentName: doc?.si?.siAttachmentName,  // Added siAttachmentName
  //         }));
  //       }
  //     });
  //   }, 800);
  ...
  // buildForm(data?) {
  //   this.instructionData = data?.instructionId;

  //   this.siAttachmentName = data?.si?.siAttachmentName;
  //   this.finalPrintAttachmentName = data?.mblOriginal?.finalAttachmentName;
  //   this.siAttachmentName = data?.si?.siReceivedAttachmentName;
  //   this.siAttachmentName = data?.si?.filedAttachmentName;
  //   this.siAttachmentName = data?.vgm?.vgmAttachmentName;
  //   this.siAttachmentName = data?.dg?.dgAttachmentName;
  //   this.finalPrintAttachmentName = data?.mblDraft?.firstPrintAttachmentName;
  ...
  open(key, data, type) {
    const modalRef = this.modalService.open(NewDocumentComponent, {
      ariaLabelledBy: 'modal-basic-title',
      backdrop: 'static',
      keyboard: false,
      centered: true,
  ...
  mail(key, doc, type) {
    const modalRef = this.modalService.open(MailsendComponent, {
      ariaLabelledBy: 'modal-basic-title',
      backdrop: 'static',
      keyboard: false,
      centered: true,
    ...
    if (this.commonService.downloadDocuments) {
      modalRef.componentInstance.getList?.subscribe((res: any) => {
        if (res) {
          this.getData();
        }
  ...
  uploadfile() {
    const modal = this.modalService.open(DocumentUpload1Component, { centered: true });
    modal.componentInstance.documentTypeList =[
      // { nzValue:"Shipping Bill" ,nzLabel:"Shipping Bill"},
      // {nzValue:"Bill of Lading" ,nzLabel:"Bill of Lading"},
      // {nzValue:"Shippingline DO" ,nzLabel:"Shippingline DO"},
      {nzValue:"Other Document" ,nzLabel:"Other Document"},
      // {nzValue:"Carbon Offset Certificate" ,nzLabel:"C/ffset Certificate"},
    ]
    modal.componentInstance.bookingid = this.urlParam?.id;
    modal.componentInstance.setDocumentName.subscribe((res: any) => {
      if (res) {
        let payload = {
          ...
          tags: [],
          Doc:res?.file?.name,
          documentURL: res?.file?.name,
          refType: 'transport',
          documentId: '',
          refId: this.urlParam?.id,
          isActive: true,
          batchId : this.urlParam?.id,
          batchNo : '',
          "tenantId": '1',
        ...
        this.commonService.uploadDocuments('document', payload).subscribe(res => {
