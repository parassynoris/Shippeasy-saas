...
export class StateMasterComponent implements OnInit {
  ...
  constructor(
    ...
    private fb: FormBuilder,private commonfunction: CommonFunctions,
  ...
  this.search_status = this.search_status?.trim();
   ...
   payload.from = 0,
    this.commonService.getSTList('state', payload).subscribe((res: any) => {
      this.stateData = res.documents?.map((s: any,index) => {
        return{
          ...s,
          id:index+1
        }
  ...
  countrySelect(countryISOCode) {
    let selectedCountry = this.countryList.find(
      (country) =>
        country?.countryISOCode === countryISOCode.target.value
    );
    this.addStateForm.controls.countryId.setValue(
  ...
  changeStatus(data, i) {
    this.commonService.UpdateToST(`state/${data.stateId}`,{ ...data, status: !data?.status },
      ...
      .subscribe(
        (res: any) => {
          if (res) {
            this.stateData[i].status = !data?.status;
  ...
  exportAsExcelFile(): void {
    ...
    const fileName = 'state-master.xlsx';
    /* save to file */
    XLSX.writeFile(myworkbook, fileName);
  }
