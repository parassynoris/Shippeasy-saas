...
@Component({
  selector: 'app-ex-bond-billentrys',
  templateUrl: './ex-bond-bill-entrys.component.html',
  styleUrls: ['./ex-bond-bill-entrys.component.scss']
})
export class ExBondBillEntrysComponent implements OnInit {
  _gc = GlobalConstants;
    @Input() batchDetail;
  ...
  filtersModel = [];
   exbondbillentryData: any;
    ...
    addexbondbillentry(): void {
      const { id, key } = this.urlParam;
      const url = `/warehouse/main-ware-house/details/${id}/${key}/add`;
      this.router.navigate([url]);
    }
    onOpenEdit(id: string): void {
      const { id: warehousedataentryId, key } = this.urlParam;

      const url = `/warehouse/main-ware-house/details/${warehousedataentryId}/${key}/${id}/edit`;
      this.router.navigate([url]);
    }
    ...
    onDeleteDispatch(id: string): void {
      this.commonService.deleteST('exbondbillentry/' + id).subscribe((data) => {
        if (data) {
          this.notification.create('success', 'Deleted Successfully', '');
        }
        setTimeout(() => {
          this.getWareHouseExBondBillEntry(this.urlParam.id);
        }, 1000);
      });
    }
    blMasterStatus: any = [];
    ...
    getWareHouseExBondBillEntry(id: string): void {
    ...
    searchGlobal() {
      ...
      let shouldArray = [];
      shouldArray.push(
        { exBeNo: { $regex: query, $options: 'i' } },
        { beNoDate: { $regex: query, $options: 'i' } },
        { chaLedger: { $regex: query, $options: 'i' } },
        { grossQty: { $regex: query, $options: 'i' } },
    ...
    this.commonService.getSTList("exbondbillentry", payload)?.subscribe((data) => {
      this.exbondbillentryData = data.documents;
      this.dataSource = new MatTableDataSource(
