...
@Component({
  selector: 'app-gate-in-data',
  templateUrl: './gate-in-data.component.html',
  styleUrls: ['./gate-in-data.component.scss']
})



export class GateInDataComponent implements OnInit, AfterViewInit {
  _gc = GlobalConstants;
  @Input() batchDetail;
  ...
  printData(element) {
    let reportpayload = { "parameters": { "warehousegateinentryId": element.warehousegateinentryId } };
  ...
  onOpenEdit(warehousedispatchId: string): void {
    ...
    if (record) {
      const url = `/warehouse/main-ware-house/details/${batchId}/${key}/${record.warehousegateinentryId}/edit`;
      this.router.navigate([url]);
    } else {
  ...
  // Alternative method if you want to pass the warehousegateinentryId
  ...
  onDelete(element: WarehouseGateOutRecord): void {
    this.commonService
      .deleteST('warehousegateinentry/' + element.warehousegateinentryId)
      .subscribe((data) => {
        if (data) {
 ...
 export() {
    ...
    console.log('Exporting data:', modifiedTableData);
     const columnMap = {
    '#': 'id',
    dispatchDate: 'gatePassNumber',
    SuerveyourName: 'purchaser',
    dispatchPackage: 'truckNumber',
    transporterLedger: 'containerNumber'
  };
