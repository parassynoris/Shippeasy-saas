...
@Component({
  selector: 'app-ware-house-packing',
  templateUrl: './ware-house-packing.component.html',
  styleUrls: ['./ware-house-packing.component.scss']
})
export class WareHousePackingComponent implements OnInit {
  _gc = GlobalConstants;
    ...
    onDeleteDispatch(id: string): void {
      this.commonService.deleteST('warehousepacking/' + id).subscribe((data) => {
        if (data) {
    ...
    getWarehouseDispatchById(id: string): void {
      ...
      this.commonService
        .getSTList('warehousepacking', payload)
        .subscribe((res) => {
