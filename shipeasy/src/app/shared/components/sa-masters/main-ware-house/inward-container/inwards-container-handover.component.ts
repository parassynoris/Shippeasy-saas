...
@Component({
  selector: 'app-inwards-container-handover',
  templateUrl: './inwards-container-handover.component.html',
  styleUrls: ['./inwards-container-handover.component.scss']
})
export class InwardsContainerHandoverComponent implements OnInit {
  _gc = GlobalConstants;
    @Input() batchDetail;
    ...
    addinwardcontainerhandover(): void {
      const { id, key } = this.urlParam;
      const url = `/warehouse/main-ware-house/details/${id}/${key}/add`;
      this.router.navigate([url]);
    }
  onOpenEdit(id: string, mode?: 'clone' | 'edit'): void {
    const { id: warehousedataentryId, key } = this.urlParam;
    if (mode === "clone") {
      const url = `/warehouse/main-ware-house/details/${warehousedataentryId}/${key}/${id}/clone`;
      this.router.navigate([url]);
    }
    else {
    ...
    onDeleteDispatch(id: string): void {
      this.commonService.deleteST('inwardcontainerhandover/' + id).subscribe((data) => {
        if (data) {
