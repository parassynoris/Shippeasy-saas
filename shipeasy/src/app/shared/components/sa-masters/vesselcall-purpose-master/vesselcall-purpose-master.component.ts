...
@Component({
  selector: 'app-vesselcall-purpose-master',
  templateUrl: './vesselcall-purpose-master.component.html',
  styleUrls: ['./vesselcall-purpose-master.component.css'],
})
export class VesselcallPurposeMasterComponent implements OnInit {
  callData: any = [];
  ...
  callMasters() {
    ...
    let newdata = {
      ...
      orgId: '',
      tenantId: '',
      typeParentType: 'vessel-call-purpose-master',
      typeDescription: 'Vessel Call Purpose',
      typeRef: 'vessel-purpose',
      typeRefId: '4',
      status: true,
    };
  ...
  search() {
    ...
    if (this.purpose) {
      mustArray.push({
        wildcard: {
          'typeName': "*" + this.purpose.toLowerCase() + "*"
        },
      });
    }
  ...
  exportAsExcelFile(): void {
    ...
    this.callData.map((row: any) => {
      storeEnquiryData.push({
        'Purpose of Call': row._source?.typeName,
      });
    });
    ...
    const fileName = 'Vessel-call-Purpose-master.xlsx';
    /* save to file */
