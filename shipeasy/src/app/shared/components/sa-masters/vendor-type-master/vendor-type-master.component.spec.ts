...
const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => VendorTypeMasterComponent),
  multi: true
};
...
describe('VendorTypeMasterComponent', () => {
  ...
  let mockSaMasterService: jasmine.SpyObj<SaMasterService>
    ...
    TestBed.configureTestingModule({
      declarations: [VendorTypeMasterComponent, MockTranslatePipe,],
  ...
  it('should call alert on delete', () => {
    spyOn(window, 'alert');
  ...
  it('should call alert when deleteclause() is called', () => {
    spyOn(window, 'alert');
    const id = { id: 1 }; // Mock id object if needed
    component.deleteclause(id);
    expect(window.alert).toHaveBeenCalledWith('Item deleted!');
  });


  it('should ensure deleteclause() works with different id formats', () => {
    spyOn(window, 'alert');
    const id = { id: 2 };
    component.deleteclause(id);
    expect(window.alert).toHaveBeenCalledWith('Item deleted!');
    ...
    const idString = '3';
    component.deleteclause(idString);
    expect(window.alert).toHaveBeenCalledWith('Item deleted!');
  });
