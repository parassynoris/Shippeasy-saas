...
describe('NotificationComponent', () => {
  ...
  it('should initialize and call getDepartmentList', () => {
    spyOn(component, 'getDepartmentList').and.callThrough();
    component.ngOnInit();
    expect(component.getDepartmentList).toHaveBeenCalled();
  });

  it('should handle error when fetching department list', fakeAsync(() => {
    commonServiceMock?.getSTList.and.returnValue(throwError({ error: { Error: 'Error' } }));
    component.getDepartmentList();
    tick();
    expect(component.departmentListForHeader.length).toBe(0);
    expect(component.departmentList.length).toBe(0);
  }));
});
