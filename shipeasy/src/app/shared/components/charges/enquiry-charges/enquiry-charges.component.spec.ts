...
import { EnquiryChargesComponent } from './enquiry-charges.component';
...
const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => EnquiryChargesComponent),
  multi: true
};

describe('EnquiryChargesComponent', () => {
  let component: EnquiryChargesComponent;
  let fixture: ComponentFixture<EnquiryChargesComponent>;
  ...
  let mockSaMasterService: jasmine.SpyObj<SaMasterService>
    ...
    TestBed.configureTestingModule({
      declarations: [EnquiryChargesComponent, MockTranslatePipe,],
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(EnquiryChargesComponent);
  ...
  it('should emit CloseNew event when onClose is called', () => {
    spyOn(component.CloseNew, 'emit');
    component.onClose('some data');
    expect(component.CloseNew.emit).toHaveBeenCalledWith('some data');
  });

  it('should emit CloseNew event with correct data when onClose is called', () => {
    spyOn(component.CloseNew, 'emit');
    component.onClose('close data');
    expect(component.CloseNew.emit).toHaveBeenCalledWith('close data');
  });
  it('should initialize form with default values', () => {
    expect(component.newenquiryForm.controls['chargeGroup'].value).toBe('');
    expect(component.newenquiryForm.controls['chargeName'].value).toBe('');
  });

  it('should emit CloseNew event on closing', () => {
    spyOn(component.CloseNew, 'emit');
    component.onClose('close');
    expect(component.CloseNew.emit).toHaveBeenCalledWith('close');
  });

  it('should sort array by key', () => {
    const array = [{ name: 'b' }, { name: 'a' }];
    const sortedArray = component.sort(array, 'name');
    expect(sortedArray).toEqual([{ name: 'a' }, { name: 'b' }]);
  });

  it('should mark the form as invalid when required fields are empty', () => {
    component.newenquiryForm.controls.chargeName.setValue('');
    component.newenquiryForm.controls.currency.setValue('');
    expect(component.newenquiryForm.valid).toBeFalsy();
  });

  
  it('should emit event when closing the modal', () => {
    spyOn(component.CloseNew, 'emit');
    component.onClose('closed');
    expect(component.CloseNew.emit).toHaveBeenCalledWith('closed');
  });
