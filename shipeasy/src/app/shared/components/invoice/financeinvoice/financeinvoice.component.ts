...
@Component({
  selector: 'app-financeinvoice',
  templateUrl: './financeinvoice.component.html',
  styleUrls: ['./financeinvoice.component.scss']
})
export class FinanceInvoiceComponent implements OnInit {
  ...
  bankModel: any;
  Documentpdf: any;
  invoiceTypeList = [{ name: "Tax", value: 'Tax' },
  ...
  constructor(
    ...
    this.editinvoiceForm = this.formBuilder.group({
      ...
      invoiceTag: ['sellerInvoice', [Validators.required]],
      bill_to: [''],
      billToBranch: [''],
      sez_type: [''],
      coName: [''],
  ...
  ngOnInit(): void {
    ...
    this.currentUrl = this.router.url?.split('?')[0].split('/')[4]

    if (this.currentUrl === 'showInvoice') {
    ...
    this.setValidation();
    if (this.id) {
      this.getInvoiceDetailsById(this.id);
    }
    ...
    this.getChargeControls().forEach(element => {
      if (element.value.enquiryitemId == '' || element.value.enquiryitemId == null) {
        let charge = {
          ...
          enqDate: this.batchDetails?.enquiryDetails?.basicDetails?.enquiryDate || '',
          stcQuotationNo: this.batchDetails?.stcQuotationNo?.toString() || '',
          enqType: this.batchDetails?.enquiryDetails?.basicDetails?.enquiryTypeId || '',
  ...
  isViewMode(): boolean {
    return this.currentUrl.includes('showInvoice');
  }
  ...
  onSave(evt, flag) {
    ...
    if (this.editinvoiceForm.valid) {
      const newInvoice = {
        ...
        "batchNo": this.batchNoList?.find((x) => x?.batchId == this.editinvoiceForm.value.batchNo)?.batchNo || '',
        "batchId": this.editinvoiceForm.value.batchNo,
        "invoiceType": "B2B",
  ...
  BlList: any;
  changeBL(e) {
    this.BlList = this.blData.find(x => x?.blId == this.f.bl.value) || ''
  }
  getblData() {
    let payload = this.commonService.filterList()
    payload.query = {
  ...
  getPartyList() {
    ...
    this.commonService.getSTList("partymaster", payload).subscribe((data: any) => {
      ...
      this.shipperDetails = data.documents?.filter((x) => x?.partymasterId === this.batchDetails?.enquiryDetails?.basicDetails?.shipperId)
    ...
    this.commonService.getSTList('invoice', payload).subscribe((data) => {
      ...
      this.billToValue = invoiceData?.invoiceToGst,
        this.editinvoiceForm.patchValue({
          ...
          invoiceTag: invoiceData?.type || 'sellerInvoice',
  ...
  getCustomerDetails() {
    if (this.type == 'buyerInvoice') {
      ...
      this.customerData = {
        branch: customerData?.branch_name,
        panNo: customerData?.pinCode,
        tax_number: customerData?.tax_number,
        address: customerData?.branch_address + ',' + customerData?.branch_address + ',' + customerData?.branch_city + ',' + customerData?.branch_state + ',' + customerData?.branch_countryName + ',' + customerData?.pinCode,
      }
    } else {
      let customerData = this.branchList?.filter(x => x?.branchId === this.editinvoiceForm?.get("billToBranch").value)[0] || ''
      this.customerData = {
        branch: customerData?.branchName,
        panNo: customerData?.addressInfo?.postalCode,
        tax_number: customerData?.taxId,
        address: customerData?.addressInfo?.address + ',' + customerData?.addressInfo?.cityName + ',' + customerData?.addressInfo?.stateName + ',' + customerData?.addressInfo?.countryName + ',' + customerData?.addressInfo?.postalCode,
      }
    }
  ...
  getBatchByID(e) {
    ...
    let payload = this.commonService.filterList()
    payload.query = {
      "batchId": this.editinvoiceForm.get("batchNo").value
    }
    ...
    this.commonService.getSTList('batch', payload)
      .subscribe((data: any) => {
        ...
        this.getEnquiry(this.batchDetails);
        this.getPartyList()
        this.getblData()
        if (!this.id) {
          this.getCharges();
  ...
  cancelIRN(data) {
    this.commonService.getGeneretINR(`cancel-from-einvoicing/${data.invoiceId}`, {})
      .subscribe((data) => {
        if (data) {
          this.notification.create('success', 'Invoice Generated Successfully', '')
          this.getInvoiceDetailsById(this.route.snapshot.params['id']);
        }
      },
        (error) => {
          if (error.error?.isMultipleError) {
  ...
  invoiceTagSelect(e) {
    ...
    this.type = this.editinvoiceForm.get('invoiceTag').value
    this.getBankList(this.quotationDetails?.branchId)
    this.setValidation()
  }
}
