...
describe('NewPaymentComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
   ...
   mockApiService = jasmine.createSpyObj('ApiService',['getListByURL']);
    TestBed.configureTestingModule({
      declarations: [NewPaymentComponent, MockTranslatePipe],
  ...
  it('should toggle "show more"', () => {
    expect(component.isShowMore).toBeFalse(); // Initially false
    component.showMore(); // Toggle
    expect(component.isShowMore).toBeTrue(); // After toggling, should be true
  });

  it('should toggle "show more" state', () => {
    expect(component.isShowMore).toBeFalse(); // Initially false
    component.showMore(); // Toggle
    expect(component.isShowMore).toBeTrue(); // After toggling, should be true
    component.showMore(); // Toggle again
    expect(component.isShowMore).toBeFalse(); // After toggling again, should be false
  });
  
  it('should emit CloseInvoiceSection event', () => {
    // Spy on the CloseInvoiceSection EventEmitter
    spyOn(component.CloseInvoiceSection, 'emit').and.callThrough();
  
    // Call the method
    component.onCloseInvoice();
  
    // Expectations
    expect(component.CloseInvoiceSection.emit).toHaveBeenCalled(); // Ensure the emit function was called
    expect(component.CloseInvoiceSection.emit).toHaveBeenCalledWith(undefined); // Ensure it was called with undefined as argument
  });
