...
const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => ConditionPopupComponent),
  multi: true
};
...
describe('ConditionPopupComponent', () => {
  ...
  let mockCommonService: jasmine.SpyObj<CommonService>;
    ...
    TestBed.configureTestingModule({
      declarations: [ConditionPopupComponent, MockTranslatePipe,],
  ...
  it('should create the ConditionPopupComponent', () => {
    expect(component).toBeTruthy();
  });

  it('should initialize the form with the correct form controls', () => {
    expect(component.addRuleForm.contains('variableDetails')).toBeTruthy();
    const variableDetails = component.addRuleForm.get('variableDetails') as FormArray;
    expect(variableDetails.length).toEqual(0); // Or assert based on initial variable list length
  });

  it('should add a new variable to the form', () => {
    component.addNewVariable();
    const variableDetails = component.addRuleForm.get('variableDetails') as FormArray;
    expect(variableDetails.length).toEqual(1);
  });

  it('should add a new variable with correct controls and values', () => {
    component.addNewVariable();
    const variableDetails = component.addRuleForm.get('variableDetails') as FormArray;
    const newVariableFormGroup = variableDetails.at(variableDetails.length - 1) as FormGroup;
    expect(newVariableFormGroup.get('variable').value).toEqual('');
    expect(newVariableFormGroup.get('condition').value).toEqual('');
    expect(newVariableFormGroup.get('value').value).toEqual('');
  });
