...
echo "${GHCR_TOKEN}" | docker login "${REGISTRY:-ghcr.io}" \
  --username github-actions --password-stdin

# ── 2. Pull new images ────────────────────────────────────────────
log "Pulling backend image  → ${BACKEND_IMAGE}:${BACKEND_TAG:-latest}"
docker pull "${BACKEND_IMAGE}:${BACKEND_TAG:-latest}"

log "Pulling frontend image → ${FRONTEND_IMAGE}:${FRONTEND_TAG:-latest}"
docker pull "${FRONTEND_IMAGE}:${FRONTEND_TAG:-latest}"

# ── 3. Update .env with new image tags ────────────────────────────
# CI passes the exact tag; the compose file reads BACKEND_TAG / FRONTEND_TAG.
cd "$DEPLOY_DIR"
grep -v '^BACKEND_TAG\|^FRONTEND_TAG' .env > .env.tmp || true
{
  echo "BACKEND_TAG=${BACKEND_TAG:-latest}"
  echo "FRONTEND_TAG=${FRONTEND_TAG:-latest}"
} >> .env.tmp
mv .env.tmp .env
log "Updated .env with new image tags"

# ── 4. Rolling restart — zero-downtime ────────────────────────────
# Restart only the services whose images changed; leave mongo untouched.
log "Restarting backend service..."
docker compose -f "$COMPOSE_FILE" up -d --no-build --no-deps --pull never backend

log "Restarting frontend service..."
docker compose -f "$COMPOSE_FILE" up -d --no-build --no-deps --pull never frontend
